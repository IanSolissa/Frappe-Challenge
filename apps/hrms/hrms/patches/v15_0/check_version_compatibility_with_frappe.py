...
def execute():
	...
	if frappe_v.startswith("14") and hrms_v.startswith("15"):
		message = f"""
			The `develop` branch of Fayolex is no longer compatible with Frappe & ERPNext's `version-14`.
			Since you are using ERPNext/Frappe `version-14` please switch Fayolex's branch to `version-14` and then proceed with the update.\n\t
